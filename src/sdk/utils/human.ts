...
import {
  fullName as generateFullName,
  firstName as generateFirstName,
  lastName as generateLastName,
} from "full-name-generator";
...
export async function humanScroll(
  ...
  if (logger)
    logger(`Scrolled ${direction} ${Math.abs(scrollDistance)} pixels`);
}
...
export async function humanMouseMove(
  ...
  try {
    ...
    const targetX =
      boundingBox.x + randomInt(5, Math.max(5, boundingBox.width - 5));
    const targetY =
      boundingBox.y + randomInt(5, Math.max(5, boundingBox.height - 5));
    ...
    if (logger)
      logger(
        `Mouse movement failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    return false;
  }
}
...
export async function humanType(
  ...
  try {
    ...
    if (logger)
      logger(
        `Typing failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    return false;
  }
}
...
export async function handleCookieBanner(
  ...
  try {
    ...
    const cookieDialogSelector =
      'div[role="dialog"].cookie-dialog, div.cookie-dialog';
    ...
    for (const buttonSelector of buttonSelectors) {
      try {
        ...
        if (logger)
          logger(
            `Failed to click ${buttonSelector}: ${error instanceof Error ? error.message : String(error)}`,
          );
      }
    }
    ...
    if (!buttonClicked) {
      ...
      if (logger)
        logger(
          "Could not click any cookie banner button, setting cookie directly",
        );
      await frame.evaluate(() => {
        document.cookie =
          "consent={%22functionality_storage%22:true%2C%22analytics_storage%22:false%2C%22ad_storage%22:false%2C%22personalization_storage%22:false}; path=/; max-age=31536000; domain=embed.bitrefill.com; SameSite=None; Secure; Partitioned";
    ...
    if (logger)
      logger(
        `Cookie banner handling failed: ${error instanceof Error ? error.message : String(error)}`,
      );
  }
}
...
export async function humanClickLocator(
  ...
  try {
    ...
    if (logger)
      logger(
        `Locator click failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    return false;
  }
}
      ...
      case 3:
        ...
        try {
          ...
          if (elements.length > 0) {
            ...
            if (logger)
              logger(`Moving mouse to random element: ${randomElement}`);
          ...
          if (logger)
            logger(
              `Random exploration failed: ${error instanceof Error ? error.message : String(error)}`,
            );
        }
        break;
    }
